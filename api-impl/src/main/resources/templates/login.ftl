...
<form action="/user/tologin" id="myform" class="login-form" method="post">
    <div class="input-phone">
        <input type="tel" maxlength="11" id="user_phone" name="phone" placeholder="请输入手机号" value="<#if phone?exists>${phone}</#if>"
        ...
        <#if count?exists>
            <#if (count>3)>
                <div style="position: relative; margin-top: 0.5333333333rem;;">
                    ...
                    <a href="javascript:changeImg();" style="position: absolute; top:  0; right: 0;"><img width="200" height="80" class="code-btn-box" id="checkcode" src="/code/imageCode"/></a>
                </div>
            </#if>
        </#if>
    <input class="code-text" type="number" name="code" maxlength="4" placeholder="请输入验证码">
...
<script>
    ...
    <#if count?exists>
        ...
        $("#log_in_button").click(function () {
            ...
            if ($("#image_code").val() == '' && $("#image_code").val() == null) {
    ...
    $("#phone_code").click(function () {
        ...
        if (user_phone.trim() == null || user_phone.trim() == '') {
            ...
            return;
        } else if (user_phone.match("^[0](13|15)[0-9]{9}$")) {
