...
<div class="module-bottom-fill">
    <section class="primary-message">
        ...
        <ul class="primary-item">
            <li>
                <p>销售状态：待售</p>
                <p>最新开盘：<#if discript['opened_time']?exists>${discript['opened_time']}<#else>暂无数据</#if></p>
                <p>交房时间：<#if discript['deliver_time']?exists>${discript['deliver_time']}<#else>暂无数据</#if></p>
                <p>参考均价：<#if discript['average_price']?exists>${discript['average_price']}元/㎡<#else>暂无数据</#if></p>
            </li>
        </ul>
    </section>
</div>
...
<div class="module-bottom-fill">
    <section class="primary-message">
        ...
        <ul class="primary-item">
            <li>
                <p>建筑类型：<#if discript['building_type']?exists>${discript['building_type']}<#else>暂无数据</#if></p>
                <p>装修类型：<#if discript['redecorate_type']?exists>${discript['redecorate_type']}<#else>暂无数据</#if></p>
                <p>产权年限：<#if discript['building_life']?exists>${discript['building_life']}<#else>暂无数据</#if></p>
                <p>占地面积：<#if discript['ground_area']?exists>${discript['ground_area']}㎡<#else>暂无数据</#if></p>
                <p>建筑面积：<#if discript['purpose_area']?exists>${discript['purpose_area']}㎡<#else>暂无数据</#if></p>
                <p>容积率：<#if discript['dimension']?exists>${discript['dimension']}<#else>暂无数据</#if></p>
                <p>绿化率：<#if discript['virescencerate']?exists&&discript['virescencerate']?number gt 0>${discript['virescencerate']?string("#.##")}%<#else>暂无数据</#if></p>
                <p>规划户数：<#if discript['totaldoor']?exists>${discript['totaldoor']}<#else>暂无数据</#if></p>
                <p>规划车位：<#if discript['park_space']?exists>${discript['park_space']}<#else>暂无数据</#if></p>
    ...
    <section class="primary-message">
        ...
        <ul class="primary-item">
            <li>
                ...
                <#--<p>燃气：<#if discript['heating_type']?exists>${discript['heating_type']}<#else>暂无数据</#if></p>-->
            </li>
        </ul>
    </section>
</div>
