...
<#if adcpc?exists><section class="recommend-list">
    <ul>
        ...
        <li><a href="${router_city('/esf/'+itemdetail['houseId']?c+'.html?cpc3esf_detail')}" class="list-item">
            <div class="clear">
...
<a class="bottom-link-box" href="http://m.toutiaofangchan.com/bj/esf?=cpc3esf_two&beginPrice=0.0&endPrice=200.0">
...
<script>
    ...
    window.onscroll = function(){
        ...
        if(getScrollTop() + getClientHeight() >= getTotalHeight() + addHeight) {
            window.location.href = "http://m.toutiaofangchan.com/bj/esf?=cpc3esf_bottom&beginPrice=0.0&endPrice=200.0";
        }
    };
    //获取滚动条当前的位置
