...
<script>
    function orientationState(){if(window.orientation==180||window.orientation==0){document.documentElement.className=""}if(window.orientation==90||window.orientation==-90){document.documentElement.className="none";alert("请使用竖屏查看！")}}window.addEventListener("onorientationchange" in window?"orientationchange":"resize",orientationState,false);
</script>